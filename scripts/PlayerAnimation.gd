...
func _ready() -> void:
	...
	# Crouch should play once and hold the last frame as the idle pose
...
func update_animation_state(velocity: Vector3, is_on_floor: bool, is_sprinting: bool, is_crouching: bool = false) -> void:
	...
	if not is_on_floor:
		...
		# Detect un-crouch: play crouch animation in reverse to stand back up
		if _was_crouching and not is_crouching:
			...
			if _is_moving:
				...
				play_animation("crouch", crouch_speed)
		elif _is_moving:
			if _is_sprinting:
